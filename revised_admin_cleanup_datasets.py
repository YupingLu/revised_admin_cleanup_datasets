...
def main():
    ...
    if 'database_connection' in config_dict:
        database_connection = config_dict['database_connection']
    else:
        database_connection = "sqlite:///%s?isolation_level=IMMEDIATE" % config_dict["database_file"]
    ...
    administrative_delete_datasets(
        app, cutoff_time, options.days, tool_id=options.tool_id, config=config,
...
def administrative_delete_datasets(app, cutoff_time, cutoff_days,
                                   tool_id, config, email_only=False, info_only=False):
